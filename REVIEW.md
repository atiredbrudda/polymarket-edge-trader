...
- ✅ Pattern-label validation completed 2026-05-07: stratified n=32 sample (B1 high-confidence ×7, B2 tpr-boundary ×10, B3 trades-boundary ×10, B4 hold-time ×5). All 32 confirmed bot/MM behavior — 77+ trades/hour and 20+ trades/position on every record. Borderline cases cluster at tpr 20-25 boundary; none are false positives. Precision 32/32 = 100% (threshold was ≥28/32).
- ✅ Materialization shipped 2026-05-07 (commit `79599fd`): `traders.is_bot` column + `bot_flag_log` audit table. `scripts/refresh_bot_flags.py` runs after `score` each cron cycle — promotes new matches, demotes after 3 consecutive non-matches, rate-limited to 20 changes/run. `load_bot_set` now reads indexed column (was 2-3s behavioral subquery). Initial population: 127 traders flagged.
- 📋 **Re-check due ~2026-05-14:** re-run the labeling sample to confirm filter is stable and no false positives crept in after a week of live refresh cycles. If any trader flipped is_bot=0 in `bot_flag_log`, investigate before expanding the filter.
