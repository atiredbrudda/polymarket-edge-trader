...
2. Merges worker branch to main: `git checkout main && git merge --no-ff worker/XX-... --no-verify`
3. Updates STATE.md (current phase/plan, progress bar, handoff notes, last updated)
4. Updates ROADMAP.md phase status if last plan of a phase
5. Commits STATE.md, REVIEW_QUEUE.md updates: `git add -f .planning/... && git commit --no-verify`
