...
- **Tests:** pytest ✓ (100/100 — 3 new DB-first tests + 97 existing)
- **Files in scope:**
  - `src/polymarket_analytics/db/schema.py` — migration adds `clob_token_ids TEXT` to markets table
  - `src/polymarket_analytics/commands/ingest_events.py` — stores `clobTokenIds` as JSON during market fetch
  - `src/polymarket_analytics/commands/classify_tokens.py` — reads from DB, async/API dependency removed
  - `tests/test_classify_tokens_db.py` (NEW) — 3 tests: CTDB-01 (no API call), CTDB-02 (empty markets error), CTDB-03 (NULL clob fallback)
  - `tests/test_integration.py` — TCAT-04 updated to DB-first (mock removed)
- **Reviewer notes:** Clean pass. API call eliminated. Minor: `outcome` column used as outcomes list — resolved markets get `market_type=categorical` instead of binary, but both tokens still inserted correctly and doesn't affect pipeline logic. No reviewer fixes required. **Ready to merge.**
