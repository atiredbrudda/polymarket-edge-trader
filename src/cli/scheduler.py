...
def _generate_sweep_debug_report(session_factory: sessionmaker, stats: dict) -> None:
    ...
    try:
        with session_factory() as session:
            ...
            sample_traders = (
                random.sample(trader_addresses, sample_size) if trader_addresses else []
            )
...
def run_sweep(
    ...
    try:
        ...
        logger.info(
            f"Ingestion complete: {stats['markets_ingested']} markets, {stats['traders_discovered']} traders"
        )
    except Exception as e:
    ...
    try:
        ...
        with session_factory() as session:
            ...
            stats["scores_computed"] = sum(
                len(entries) for entries in leaderboards.values()
            )
        logger.info(
            f"Scoring complete: {stats['scores_computed']} scores computed across {len(leaderboards)} games"
        )
    except Exception as e:
    ...
    if alerter and not skip_alerts:
        try:
            ...
            logger.info(
                f"Alert delivery complete: {stats['alerts_sent']} sent, {stats['alerts_failed']} failed"
            )
        except Exception as e:
            logger.error(f"Alert delivery stage failed: {e}")
    else:
        logger.info(
            "Stage 4/4: Skipping alerts (alerter not configured or skip_alerts=True)"
        )
