...
def test_convert_maker_trade():
    """Trader as maker: size=makerAmount/1e6, side=original, price preserved."""
    ...
    assert result.side == "BUY"  # Maker's side unchanged
...
def test_convert_taker_trade():
    """Trader as taker: size=takerAmount/1e6, side=OPPOSITE, price preserved."""
    from src.datasources.converters import jbecker_trade_to_api_response
    ...
    assert isinstance(result, TradeResponse)
    assert result.side == "SELL"  # Opposite of maker's BUY
...
def test_amount_6_decimal_conversion():
    ...
    trade["makerAmountFilled"] = "1500000"
    trader_address = "0xeffd76b6a4318d50c6f71a16b276c5b279445a86"  # maker
...
def test_amount_large_value():
    ...
    trade = SAMPLE_JBECKER_TRADE.copy()
    trade["makerAmountFilled"] = "1000000000"
    trader_address = "0xeffd76b6a4318d50c6f71a16b276c5b279445a86"  # maker
...
def test_maker_gets_buy_side():
    """Maker with side=BUY gets side=BUY."""
    from src.datasources.converters import jbecker_trade_to_api_response
...
def test_taker_gets_opposite_side():
    """Taker with side=BUY gets side=SELL (opposite)."""
    from src.datasources.converters import jbecker_trade_to_api_response
...
def test_case_insensitive_role_matching():
    ...
    # Should be recognized as maker despite case mismatch
    assert result.side == trade["side"]  # Maker gets original side
...
def test_asset_ticker_even_no():
    ...
    trade = SAMPLE_JBECKER_TRADE.copy()
    trade["takerAssetId"] = "789012"  # even
    trader_address = "0xabc123def456789012345678901234567890abcd"  # taker
...
def test_invalid_price_skipped():
    """Prices outside (0,1) exclusive range raise ValueError from TradeResponse validator.

    The converter should NOT catch this - it's expected behavior.
    The pipeline caller handles the exception.
    """
    from src.datasources.converters import jbecker_trade_to_api_response
    from pydantic import ValidationError

    trade = SAMPLE_JBECKER_TRADE.copy()
    trade["price"] = "1.5"  # Invalid: > 1
    trader_address = "0xeffd76b6a4318d50c6f71a16b276c5b279445a86"
